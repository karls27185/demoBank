...
/**
 * Класс BankTransactionService представляет собой сервис, который управляет банковскими транзакциями.
 * */
...
public class BankTransactionService {
    ...
    @Transactional
    public void chargeInterest(BankAccount account) {//начисляет проценты на банковский счё
    ...
    @Transactional
    public void increaseBalance(BankAccount account, BigDecimal amount) {//увеличивает баланс банковского счёта на указанную сумму
        String transactionId = UUID.randomUUID().toString();
        BankTransaction transaction = new BankTransaction(transactionId, null, account, amount, TransactionStatus.PENDING);
        try {
            transaction.deposit(amount);
    ...
    @Transactional
    public void decreaseBalance(BankAccount account, BigDecimal amount) {//уменьшает баланс банковского счёта на указанную сумму
        String transactionId = UUID.randomUUID().toString();
    ...
    @Transactional
    public void internalFundTransfer(BankAccount fromAccount, BankAccount toAccount, BigDecimal amount) {//выполняет перевод средств между двумя банковскими счетами
